...
with{
    ...
    out = line : exf:si.smooth(0.9); 
}; 

main_clock = timephasor
with{
    run = 1-(bpm>0);
    timephasor = os.hs_phasor(1.0,bpm/15/4,run);
};
...
c = vslider("Decay",0.2,0,1,0.01):si.smoo;
f = vslider("freq", 100,10,800,1) : si.smoo; 
spread = vslider("Hsprd",0,0,1,0.001): si.smooth(.8);
k = vslider("oddlvl",0.5,0,1,0.001);
n = vslider("n.o.h",4,1,32,1);
...
with{
    ...
    rezfilter(in, i) = out
        with{
            s = spread*2;
            F = (freq*pow(i+1,s)%20000) : qu.quantize(220, qu.natural): si.smoo : ba.if(freq*pow(s,i) < 20000,_,20000);
            filtering(i,nbr,in) = 
...
with {
    ...
    with{ 
            ...
            preout = env*no.noise:fi.lowpass(4,2000);
    ...
    out = vgroup("decoders",imp,tm(pass,prob));
};



// main process function 
p = sequence : rez(f,n,spread): aa.tanh1 : echo;
...
with{
    ...
    disp(i) = wa.allpass2(i*100+30*wow*i%2000,1+37*dfactor,0) : wa.allpass2(i*100-70*wow*i%2000,1+20*dfactor,0);
