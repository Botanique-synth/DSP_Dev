...
class DspFaust
{
    ...
    public:
        ...
        // Compute one block of audio samples. Bypasses the audio driver.
        // Call this instead of start() when you want to feed audio to
        // a custom output (e.g. Web Audio API ScriptProcessorNode).
        //
        // #### Arguments
        //
        // * `count`: number of frames to compute
        // * `inputs`: array of input channel buffers (can be nullptr if no inputs)
        // * `outputs`: array of output channel buffers to fill
        //--------------------------------------------------------
        void compute(int count, float** inputs, float** outputs);
